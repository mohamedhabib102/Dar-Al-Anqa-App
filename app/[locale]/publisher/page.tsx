...
const Author: React.FC = () => {
    ...
    const getAuthorBooks = async () => {
        ...
        try {
            const res = await api.get(`/api/Books/GetAllBooksByPublisher?User_ID=${userData.userId}`);
            setBooks(res.data);
        } catch (error) {
            console.log(error);
        }
                                    ...
                                    <div key={request.withdraw_Id} className="bg-white rounded-lg shadow-md p-5 hover:shadow-xl transition-all duration-300 border border-gray-100">
                                        ...
                                        <div className="space-y-2 text-sm">
                                            ...
                                            {request.accountNumber && (
                                                <div className="flex justify-between">
                                                    <span className="text-gray-600">{locale === "ar" ? "رقم الحساب" : locale === "en" ? "Account Number" : "Numéro de compte"}:</span>
                                                    <span className="font-medium text-gray-700">{request.accountNumber}</span>
                                                </div>
                                            )}
                                            <div className="flex justify-between text-xs">
                                                <span className="text-gray-500">{locale === "ar" ? "تاريخ الطلب" : locale === "en" ? "Order Date" : "Date de commande"}:</span>
